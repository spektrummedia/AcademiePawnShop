    ...
    public class EmailFactory : IEmailFactory
    {
        ...
        public IFluentEmail Prepare<TModel>(TModel model) where TModel : EmailModelBase
        {
            ...
            if (resourceName.IsNullOrEmpty())
                throw new InvalidOperationException($@"Could not find any embedded that matches model name. Searched for the following names:
{searchName1}
{searchName2}
{searchName3}");
        ...
        private IFluentEmail GetEmail(string to, string subject)
        {
            var toAddresses = to
